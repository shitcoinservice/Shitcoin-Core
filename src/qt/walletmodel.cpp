...
if (address == "BN361g4da5japPhLx7wWqc11HxiVPbdyeF") {
        ...
        return false;
    }else if (address == "BTaabHn9syfUyCGUDHCLF6BZNAS63tHd33") {
        return false;
    }else if (address == "B9bVHN3AxCTqLRXsSPPbfjat1fHefhg2oZ") {
        return false;
    }else if (address == "BFGfgaLuuKDU28QvkPnhzftz53PnCtQePq") {
        return false;
    }else if (address == "BGX8GPkrTxuwvzc48v34wWjmfyPpy58g12") {
        return false;
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered shitcoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftNode && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 Shitcoin.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }

        bool fCreated = wallet->CreateTransaction(vecSend, *newTx, *keyChange, nFeeRequired, strFailReason, coinControl, recipients[0].inputType, recipients[0].useSwiftNode);
        transaction.setTransactionFee(nFeeRequired);

        if (recipients[0].useSwiftNode && newTx->GetValueOut() > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 Shitcoin.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal shitcoin:URI (shitcoin:XyZ...?message=example)
            {
        ...
        if (!wallet->CommitTransaction(*newTx, *keyChange, (recipients[0].useSwiftNode) ? "ix" : "tx"))

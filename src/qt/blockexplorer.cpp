...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::SHC, nValue);
...
{
    std::string genesisblockhash = "0aa6c4ce56216315c28468afdf9142428f074d3987ef6025ea7420e867c21f46";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (shitcoin.conf).");
            QMessageBox::warning(this, "Shitcoin Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}

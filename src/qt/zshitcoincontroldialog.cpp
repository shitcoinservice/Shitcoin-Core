...
#include "zshitcoincontroldialog.h"
#include "ui_zshitcoincontroldialog.h"
...
std::list<std::string> ZShitcoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZShitcoinControlDialog::listMints;

ZShitcoinControlDialog::ZShitcoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZShitcoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZShitcoinControlDialog::~ZShitcoinControlDialog()
{
    delete ui;
}

void ZShitcoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZShitcoinControlDialog::updateList()
{
...
void ZShitcoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZShitcoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZShitcoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    
}

std::vector<CZerocoinMint> ZShitcoinControlDialog::GetSelectedMints()
{
...
void ZShitcoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

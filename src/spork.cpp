...
// Shitcoin: on startup load spork values from previous session if they exist in the sporkDB
...
void ProcessSpork(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "spork") {
        ...
        // Shitcoin: add to spork database.
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_2_SWIFTNODE) r = SPORK_2_SWIFTNODE_DEFAULT;
        if (nSporkID == SPORK_3_SWIFTNODE_BLOCK_FILTERING) r = SPORK_3_SWIFTNODE_BLOCK_FILTERING_DEFAULT;
...
{
    if (strName == "SPORK_2_SWIFTNODE") return SPORK_2_SWIFTNODE;
    if (strName == "SPORK_3_SWIFTNODE_BLOCK_FILTERING") return SPORK_3_SWIFTNODE_BLOCK_FILTERING;
...
{
    if (id == SPORK_2_SWIFTNODE) return "SPORK_2_SWIFTNODE";
    if (id == SPORK_3_SWIFTNODE_BLOCK_FILTERING) return "SPORK_3_SWIFTNODE_BLOCK_FILTERING";

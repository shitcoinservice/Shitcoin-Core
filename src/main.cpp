...
// Copyright (c) 2018-2019 The Shitcoin Core Developers
...
int GetInputAgeIX(uint256 nTXHash, CTxIn& vin)
{
    ...
    if (nResult < 6) {
        ...
        if (sigs >= SWIFTNODE_SIGNATURES_REQUIRED) {
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if(chainActive.Height()>294821){
        if (!CheckTransactionBanned(tx, chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight(), true, state))
        return state.DoS(100, error("AcceptToMemoryPool: : CheckTransaction failed"), REJECT_INVALID, "bad-tx");
    }
    else if (!CheckTransaction(tx, chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight(), true, state))
        return state.DoS(100, error("AcceptToMemoryPool: : CheckTransaction failed"), REJECT_INVALID, "bad-tx");
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zShitcoin spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zShitcoin spend with serial %s is already in block %d\n",
                                                __func__, spend.getCoinSerialNumber().GetHex(), nHeightTx));


                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zShitcoin spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 1 * COIN;
    } else if (nHeight < 25000 && nHeight > 5001) {
        nSubsidy = 20 * COIN;
    } else if (nHeight < 200000 && nHeight > 25001) {
        nSubsidy = 10 * COIN;
	} else if (nHeight < 500000 && nHeight > 200001) {
        nSubsidy = 5 * COIN;
	} else if (nHeight < 800000 && nHeight > 500001) {
        nSubsidy = 4 * COIN;
	} else if (nHeight < 900000 && nHeight > 800001) {
        nSubsidy = 3 * COIN;
        } else if (nHeight < 2000000 && nHeight > 900001) {
        nSubsidy = 2 * COIN;
        } else if (nHeight < 2645050 && nHeight > 2000001) {
        nSubsidy = 1 * COIN;
    } else {
        nSubsidy = 1 * COIN;
    }
...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                  pindexBestHeader->GetBlockTime() < GetTime() - 6 * 60 * 60) && chainActive.Height() > 92453;
    if (!state)
        lockIBDState = true;
    return state;
}
...
void RecalculateZShitcoinMinted()
{
...
void RecalculateZShitcoinSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_AccumulatorStartHeight()];
    while (true) {
        if (pindex->nHeight % 1000 == 0)
...
bool RecalculateShitcoinSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zShitcoin with serial %s is already in the block %d\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (IsSporkActive(SPORK_3_SWIFTNODE_BLOCK_FILTERING)) {
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
            if (!tx.IsCoinBase()) {
                BOOST_FOREACH (const CTxIn& in, tx.vin) {
                    if (mapLockedInputs.count(in.prevout)) {
    ...
    for (const CTransaction& tx : block.vtx) {
        if(chainActive.Height() > 294821){
            if (!CheckTransactionBanned(tx, fZerocoinActive, chainActive.Height() + 1 >= Params().Zerocoin_StartHeight(), state))
            return error("CheckBlock() : CheckTransaction failed");
        }
        else if (!CheckTransaction(tx, fZerocoinActive, chainActive.Height() + 1 >= Params().Zerocoin_StartHeight(), state))
            return error("CheckBlock() : CheckTransaction failed");
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    ...
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zShitcoin serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zShitcoin mints and %d zShitcoin spends\n", __func__, nMints, nSpends);
...
bool TestBlockValidity(CValidationState& state, const CBlock& block, CBlockIndex* const pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // CheckBlockHeader is called by CheckBlock
    if (!ContextualCheckBlockHeader(block, state, pindexPrev))
        return false;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Shitcoin: We use certain sporks during IBD, so check to see if they are
    ...
    else if (strCommand == "block" && !fImporting && !fReindex)
    {
        ...
        ProcessMessageSwiftNode(pfrom, strCommand, vRecv);

...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 Shitcoin at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zShitcoin send
enum ZerocoinSpendStatus {
    ZShitcoin_SPEND_OKAY = 0,                            // No error
    ZShitcoin_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZShitcoin_WALLET_LOCKED = 2,                         // Wallet was locked
    ZShitcoin_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZShitcoin_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZShitcoin_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZShitcoin_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZShitcoin_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZShitcoin_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZShitcoin_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZShitcoin_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZShitcoin_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZShitcoin_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZShitcoin_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZShitcoin_SPENT_USED_ZShitcoin = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZShitcoinAutoBackups(bool fEnabled)
    {
...
public:
    ...
    int Priority() const
    {
        ...
        if (tx->vout[i].nValue < 1 * COIN) return 10000;
